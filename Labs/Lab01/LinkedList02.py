# 2. Re-write the function removeNode() using the LinkedList structure defined in the
# lecture materials. The function should remove a node at the specified index from the
# linked list.
...
class LinkedList:
    ...
    def remove2(self, index):
        ...
        current = self.head
        prev = None
        while self.head and count < index:
            prev = current
            current = current.next
            count += 1
        if not current:
