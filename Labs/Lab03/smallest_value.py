...
def smallest_value(node):
    ...
    leftMin = smallest_value(node.left)
    rightMin = smallest_value(node.right)
    
    currentMin = node.item
    
    # Remember to add a check to check if the node is not empty
    if leftMin is not None and leftMin < currentMin:
        currentMin = leftMin
    if rightMin is not None and rightMin < currentMin:
        currentMin = rightMin
